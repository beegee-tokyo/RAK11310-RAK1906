...
bool init_bme(void)
{
	...
	{
		APP_LOG("BME", "Could not find a valid BME680 sensor, check wiring!");
		return false;
	}
...
bool read_bme(void)
{
	...
	APP_LOG("BME", "RH= %.2f T= %.2f", (float)(humid_int / 2.0), (float)(temp_int / 10.0));
	APP_LOG("BME", "P= %d R= %d", press_int * 10, gasres_int * 10);
	APP_LOG("BME", "RH= %.2f T= %.2f", bme.humidity, bme.temperature);
	APP_LOG("BME", "P= %ld R= %ld", bme.pressure, bme.gas_resistance);
	return true;
}